...
export const { createTheme, css, getCssText, keyframes } = getNodeProcess()
  ? createStitchesMock : createStitches({ prefix: THEME_PREFIX });

const defaultVariables = {
  space: new Array(11).fill(' ').reduce((acc, _, index) => ({ ...acc, [index + 1]: `${(index + 1) * 4}px` }), {}),
  border: { radius: '4px' },
...
export const standardizeTheme = (
  ...
  if (typeof inputTheme === 'string') {
    ...
    if (!predefinedTheme) {
      throw new Error(
        `[sandpack-vue3]: invalid theme '${inputTheme}' provided.`,
      );
    }
